...
function bind(){document.addEventListener('change',onChange,true);window.addEventListener('hashchange',()=>setTimeout(()=>{if(location.hash==='#browse'){sync();updateCount()}},0));setTimeout(()=>{if(location.hash==='#browse'){sync();updateCount()}},0)}
async function init(){try{const r=await fetch('data/india-admin.json?v=1',{cache:'no-store'});if(r.ok)ADMIN=await r.json()}catch{}bind();if(location.hash==='#browse'){sync();updateCount()}}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
})();