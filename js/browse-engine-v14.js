...
function onChange(e){const s=e.target;if(!s.matches('select[data-browse]'))return;const n=s.dataset.browse,v=s.value;if(n==='State / Union Territory'&&v){filters={'State / Union Territory':v};selects().forEach(x=>{if(x!==s)x.value=''})}else if(n==='State / Union Territory'){delete filters[n]}else if(n==='District'){if(v)filters[n]=v;else delete filters[n];delete filters['Village / Town / City'];const p=selects().find(x=>x.dataset.browse==='Village / Town / City');if(p)p.value=''}else{if(v)filters[n]=v;else delete filters[n]}save();sync();updateCount()}
